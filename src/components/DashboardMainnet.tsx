...
//   - FlowFi's own contract method IDs (swap/pool/token/position/etc.)
//     won't show up here yet since those contracts aren't deployed to
//     mainnet -- activity will mostly categorize as Send/Approve/Other
//     until Swap, Token Launch, and Pools ship on mainnet (later roadmap
//     steps). Not a bug, just reflects what's actually live today.
interface Props {
  address: string;
...
function loadWeekSnapshot(address: string): { weekStart: string; value: number } | null {
  try {
    const raw = localStorage.getItem(`flowfi-portfolio-week-mainnet-${address}`);
    return raw ? JSON.parse(raw) : null;
  } catch {
    return null;
  }
...
export default function DashboardMainnet({ address, balances }: Props) {
  ...
  useEffect(() => {
    ...
    const dayKey = `flowfi-portfolio-snapshot-mainnet-${address}`;
    ...
    if (!weekSnap) {
      localStorage.setItem(`flowfi-portfolio-week-mainnet-${address}`, JSON.stringify({ weekStart: weekKey, value: total }));
