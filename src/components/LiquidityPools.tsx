...
const KNOWN_TOKENS: { symbol: string; address: `0x${string}`; color: string }[] = [
  ...
  { symbol: "EURC", address: "0x89B50855Aa3bE2F677cD6303Cec089B5F319D72a", color: "#22d3ee" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem", maxWidth: 460 }}>
      ...
      <button onClick={() => setShowCreate(!showCreate)}
        style={{ width: "100%", padding: "0.8rem", borderRadius: 12, border: "none", background: "#111a2c", color: "#67e8f9", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
        {showCreate ? "Cancel" : "+ Create New Pool"}
      </button>

      {showCreate && (
        <div style={{ background: "#0b1220", borderRadius: 16, padding: "1.25rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <select value={tokenASym} onChange={(e) => setTokenASym(e.target.value)}
              style={{ flex: 1, background: "#111a2c", border: "none", borderRadius: 10, padding: "0.65rem", color: "#f1f5f9", fontSize: 13 }}>
          ...
          {tokenASym === "CUSTOM" && (
            <div style={{ position: "relative" }}>
              <input type="text" placeholder="Token name (e.g. Doge) or address (0x...)" value={customAAddr} onChange={(e) => handleCustomASearch(e.target.value)}
                style={{ width: "100%", background: "#111a2c", border: "none", borderRadius: 10, padding: "0.65rem 0.8rem", color: "#f1f5f9", fontSize: 12, outline: "none" }} />
          ...
          <button onClick={createPool} disabled={creating}
            style={{ width: "100%", padding: "0.8rem", borderRadius: 10, border: "none", background: "linear-gradient(135deg, #22d3ee, #22d3ee)", color: "#fff", fontSize: 14, fontWeight: 700, cursor: creating ? "not-allowed" : "pointer", opacity: creating ? 0.6 : 1 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <button onClick={onToggle}
        style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", background: "#0b1220", borderRadius: 14, padding: "1rem 1.25rem", cursor: "pointer", textAlign: "left" }}>
      ...
      {expanded && (
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", paddingLeft: 4 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6 }}>
            <div style={{ background: "#111a2c", borderRadius: 10, padding: "0.6rem 0.5rem", textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.6rem" }}>
            <div style={{ background: "#111a2c", borderRadius: 10, padding: "0.7rem 0.85rem" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {swapSupported && (
              <button onClick={() => setMode("swap")}
                style={{ flex: 1, padding: "0.5rem", borderRadius: 8, border: "none", background: mode === "swap" ? "#1b2740" : "#111a2c", color: mode === "swap" ? "#67e8f9" : "#64748b", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                Swap
              </button>
            )}
            <button onClick={() => setMode("add")}
              style={{ flex: 1, padding: "0.5rem", borderRadius: 8, border: "none", background: mode === "add" ? "#1b2740" : "#111a2c", color: mode === "add" ? "#67e8f9" : "#64748b", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              Add
            </button>
            <button onClick={() => setMode("remove")}
              style={{ flex: 1, padding: "0.5rem", borderRadius: 8, border: "none", background: mode === "remove" ? "#1b2740" : "#111a2c", color: mode === "remove" ? "#67e8f9" : "#64748b", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              Remove
            </button>
          </div>

          {mode === "swap" && swapSupported && (
            <div style={{ background: "#111a2c", borderRadius: 12, padding: "1rem", display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <input type="number" min="0" placeholder="0.00" value={swapAmountIn} onChange={(e) => setSwapAmountIn(e.target.value)} disabled={isSwapLoading}
                  style={{ flex: 1, background: "#111a2c", border: "none", borderRadius: 8, padding: "0.6rem 0.8rem", fontSize: 14, color: "#f1f5f9", outline: "none" }} />
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ flex: 1, background: "#111a2c", borderRadius: 8, padding: "0.6rem 0.8rem", fontSize: 14, color: "#f1f5f9" }}>{swapEstOut}</div>
          ...
          {mode === "add" && (
            <div style={{ background: "#111a2c", borderRadius: 12, padding: "1rem", display: "flex", flexDirection: "column", gap: 8 }}>
              <input type="number" min="0" placeholder={`${resolvedSymbolA} amount`} value={amountA} onChange={(e) => setAmountA(e.target.value)} disabled={isLoading}
                style={{ background: "#111a2c", border: "none", borderRadius: 8, padding: "0.6rem 0.8rem", fontSize: 13, color: "#f1f5f9", outline: "none" }} />
              ...
              <button onClick={doAdd} disabled={isLoading}
                style={{ padding: "0.6rem", borderRadius: 8, border: "none", background: "linear-gradient(135deg, #22d3ee, #22d3ee)", color: "#fff", fontSize: 12, fontWeight: 700, cursor: isLoading ? "not-allowed" : "pointer", opacity: isLoading ? 0.6 : 1 }}>
