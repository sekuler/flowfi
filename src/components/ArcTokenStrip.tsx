...
export default function ArcTokenStrip({ chainId, label = "On Arc", onPick, initialSelected }: { chainId: number; label?: string; onPick: (t: ArcToken) => void; initialSelected?: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#EEF1FE", borderRadius: 16, padding: "9px 10px 9px 14px", marginBottom: 10 }}>
      <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, fontWeight: 700, color: "#4B5563", flexShrink: 0 }}>
        <Sparkles size={14} color="#3D5AF1" /> {label}
      </span>
        ...
        {tokens.map((t) => {
          ...
          return (
            <button key={t.address} type="button" onClick={() => { setSelected(t.address.toLowerCase()); onPick(t); }}
              style={{ display: "flex", alignItems: "center", gap: 7, flexShrink: 0, padding: "4px 12px 4px 5px", borderRadius: 999, background: "#fff", border: on ? "1.5px solid #3D5AF1" : "1px solid #D5DCF9", boxShadow: on ? "0 0 0 3px rgba(61,90,241,0.12)" : "none", cursor: "pointer", transition: "all 0.15s" }}>
      ...
      <button type="button" aria-label="Scroll tokens" onClick={() => scroller.current?.scrollBy({ left: 180, behavior: "smooth" })}
        ...
        <ChevronRight size={18} color="#3D5AF1" />
      </button>
    </div>
  );
}
