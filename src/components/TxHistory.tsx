  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        ...
        <button onClick={load} style={{ background: "#f5f3ff", border: "none", borderRadius: 8, padding: "6px 12px", color: "#64748b", fontSize: 12, cursor: "pointer" }}>↻ Refresh</button>
      </div>
  ...
  <EmptyState icon="📭" title="No transactions found" subtitle="Your activity will show up here once you start using FlowFi" />
      ...
      {!loading && filteredTxs.length > 0 && (
        <div style={{ background: "#ffffff", borderRadius: 16, overflow: "hidden" , boxShadow: "0 1px 3px rgba(124,58,237,0.08)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "80px 1fr 90px 60px 70px", gap: 8, padding: "0.6rem 1rem", background: "#f5f3ff", fontSize: 10, color: "#475569", fontWeight: 700, letterSpacing: "0.5px" }}>
            <span>TYPE</span>
            <span>TX HASH</span>
            <span>STATUS</span>
            <span></span>
            ...
            return (
              <a key={tx.hash} href={`https://testnet.arcscan.app/tx/${tx.hash}`} target="_blank" rel="noopener noreferrer"
                ...
                <button onClick={(e) => copyHash(tx.hash, e)} title="Copy hash"
                  style={{ background: "#f5f3ff", border: "none", borderRadius: 6, padding: "3px 8px", color: copiedHash === tx.hash ? "#6ee7b7" : "#64748b", fontSize: 11, cursor: "pointer", width: "fit-content" }}>
                  {copiedHash === tx.hash ? "✓" : "⧉"}
                </button>
